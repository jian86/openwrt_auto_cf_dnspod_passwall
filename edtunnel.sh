...
while [ $retries -lt $max_retries ]; do
    ...
    # 下载压缩文件
    wget -O "${DATA_DIR}txt.zip" --timeout=3 "http://zip.cm.edu.kg/"
    
    if [ $? -eq 0 ]; then
        echo "正在解压缩最新IP段${txtfile}，请稍后..."
        # 解压缩文件并获取特定文件名
        unzip -p "${DATA_DIR}txt.zip" "443/${txtfile}" > "${DATA_DIR}${CFST_f}"
        # 删除临时文件
        rm "${DATA_DIR}txt.zip"
        echo "最新IP段${txtfile}，获取完成..."
...
#$DATA_DIR/CloudflareST -url $CFST_URL -tp $CFST_port -sl $CFST_sl -dn $CFST_dn -p $CFST_p -f $DATA_DIR$CFST_f -o $DATA_DIR$CFST_o
echo "测速完毕"
echo "正在更新，请稍后..."
echo "获取优选后的ip地址"
...
while true; do
    # 使用curl获取IP地址信息
    json=$(curl -s https://ip-moe.zerodream.net/?ip=$ipAddr)
...
echo "#####################################################################################################"
echo "#####################################################################################################"
# beep -f 800 -l 100 -r 2
#####################################################################################################
logger "#####################################################################################################"
logger "EDtunnel优选通知:"
logger "开始时间${current_time}"
logger "EDtunnel域名设置为'${ip_addr_dns}'"
logger "PassWall节点设置为${substring}"
logger "丢包率${speedloss}(%)"
logger "平均延迟${speedping}(ms)"
logger "下载速度${speedtest}(MB/s)"
logger "IP区域为${country}"
logger "结束时间${shutdown_time}"
logger "执行时长${runtime}秒"
logger "#####################################################################################################"
exit ;
